...
cat <<EOF > /usr/local/etc/pkg/repos/samba.conf
samba: {
    url: "http://pkg.freebsd.org/freebsd:11:x86:64/latest/",
    mirror_type: "https",
    enabled: yes
}
EOF

/usr/sbin/pkg update -r samba
/usr/sbin/pkg install -r samba net/samba411 2> /dev/null

/usr/sbin/pkg unlock pkg
/usr/sbin/pkg unlock pfSense-2.4.5

rm -rf /usr/local/etc/pkg/repos/samba.conf
/usr/sbin/pkg update
...
fetch -o /usr/local/pkg -q https://github.com/jacksongobetti/transpotech/blob/master/samba.inc
fetch -o /usr/local/pkg -q https://github.com/jacksongobetti/transpotech/blob/master/samba.xml
...
cd /usr/local/pkg
fetch -o - -q https://github.com/jacksongobetti/transpotech/blob/master/squid_winbind_auth.patch | patch -b -p0 -f
fetch -o /usr/local/pkg -q https://github.com/jacksongobetti/transpotech/blob/master/squid.inc/squid.inc
